...
# VAD_FRAME_DURATION_MS = 30  # 30ms frames for VAD
...
# VAD_MIN_CHUNK_DURATION = 0.3  # Minimum chunk duration for VAD-based chunking
...
# # Keywords lists (existing - truncated for brevity)
# HUMAN_KEYWORDS = [
...
#     "at the tone", "after the signal", "voicemail", "voice mail",
#     "answering machine", "mailbox", "record your message",
...
# print("=" * 78)
# print(" Audio File Processor for AMD Detection with Speaker Diarization & VAD Chunking")
...
#         # Use newer parameter name 'token' instead of deprecated 'use_auth_token'
...
#             if duration >= VAD_MIN_CHUNK_DURATION:
...
# def remove_repeated_text(text: str, max_repetition: float = 0.5) -> str:
#     """
#     Remove repeated phrases/words from transcription.
#     Handles hallucinations like repeated "please leave a message" phrases.
    
#     Args:
#         text: Original transcription text
#         max_repetition: Maximum allowed repetition ratio (0-1)
    
#     Returns:
#         Cleaned text with repeated phrases removed
#     """
#     if not text or len(text.strip()) < 2:
#         return text
...
#     for word in words:
#         # Skip excessive repetition of same word
#         if word == prev_word:
#             continue
...
#     # Check for phrase-level repetition (2-3 word sequences)
#     phrases_2gram = []
#     for i in range(len(cleaned_words) - 1):
#         phrase = f"{cleaned_words[i]} {cleaned_words[i+1]}"
#         phrases_2gram.append(phrase)
    
#     # Count phrase occurrences and remove if excessive
#     phrase_counts = {}
#     for phrase in phrases_2gram:
#         phrase_counts[phrase] = phrase_counts.get(phrase, 0) + 1
    
#     # Reconstruct text, removing overly repeated phrases
#     result_words = []
#     i = 0
#     while i < len(cleaned_words):
#         # Check if next two words form a heavily repeated phrase
#         if i < len(cleaned_words) - 1:
#             phrase = f"{cleaned_words[i]} {cleaned_words[i+1]}"
#             if phrase_counts.get(phrase, 0) > 2:
#                 # Skip this phrase occurrence
#                 i += 2
#                 continue
        
#         result_words.append(cleaned_words[i])
#         i += 1
    
#     cleaned_text = " ".join(result_words)
    
#     # Preserve original casing pattern from first occurrence
#     return cleaned_text

# def chunk_segment_by_duration(segment: Dict, audio_16k: np.ndarray, 
#                              max_chunk_duration: float = 5.0, 
#                              sr: int = 16000) -> List[Dict]:
#     """
#     Further chunk a diarization segment into smaller duration-based chunks.
#     Applies VAD within the segment to find natural speech boundaries.
    
#     Args:
#         segment: Diarization segment with start/end times
#         audio_16k: Full audio array at 16kHz
#         max_chunk_duration: Maximum duration per chunk
#         sr: Sample rate (16000 Hz)
    
#     Returns:
#         List of sub-chunks with VAD-detected boundaries
#     """
#     start_sample = int(segment['start'] * sr)
#     end_sample = int(segment['end'] * sr)
#     segment_audio = audio_16k[start_sample:end_sample]
    
#     # Detect speech boundaries within segment using VAD
#     vad_segments = detect_speech_with_vad(segment_audio, sr)
    
#     if not vad_segments:
#         # No VAD segments, return original segment as single chunk
#         return [{
#             'start': segment['start'],
#             'end': segment['end'],
...
#     # Further chunk VAD segments if they exceed max_chunk_duration
#     chunks = []
#     for vad_seg in vad_segments:
#         vad_start = segment['start'] + vad_seg['start']
#         vad_end = segment['start'] + vad_seg['end']
#         vad_duration = vad_end - vad_start
        
#         if vad_duration > max_chunk_duration:
#             # Split into smaller chunks
#             num_chunks = int(np.ceil(vad_duration / max_chunk_duration))
#             chunk_size = vad_duration / num_chunks
            
#             for i in range(num_chunks):
#                 chunk_start = vad_start + i * chunk_size
#                 chunk_end = min(vad_start + (i + 1) * chunk_size, vad_end)
...
#         else:
#             chunks.append({
#                 'start': round(vad_start, 2),
#                 'end': round(vad_end, 2),
#                 'speaker': segment['speaker'],
#                 'vad_detected': True,
#                 'duration': round(vad_duration, 2)
#             })
...
#     pyAnnote => VAD + Duration Chunks => Whisper + Repeated Text Removal => Classification
#     """
#     print(f"\n{'='*80}")
...
#     # STEP 2: VAD + DURATION CHUNKING
#     print("Step 2: Applying VAD and duration-based chunking...")
#     audio_16k = load_audio_file(file_path, target_sr=16000)
...
#         chunks = chunk_segment_by_duration(seg, audio_16k, max_chunk_duration=5.0, sr=16000)
#         all_chunks.extend(chunks)
    
#     chunk_time = time.time() - chunk_start_time
#     print(f"Created {len(all_chunks)} VAD chunks in {chunk_time:.2f}s")
    
#     # STEP 3: TRANSCRIBE & CLASSIFY EACH CHUNK
#     print("Step 3: Transcribing and classifying chunks...")
#     results = []
...
#             print(f"  Chunk {i}: Failed to extract audio")
#             continue
...
#                 'vad_detected': chunk.get('vad_detected', False)
#             })
            
#             display_text = text[:60] + "..." if len(text) > 60 else text
#             print(f"  Chunk {i}: {chunk['speaker']} ({chunk['start']:.2f}s-{chunk['end']:.2f}s): '{display_text}' -> {decision}")
#         else:
#             print(f"  Chunk {i}: {chunk['speaker']} ({chunk['start']:.2f}s-{chunk['end']:.2f}s): No speech detected")
...
#         'segments': results,
#         'processing_time': diarization_time + chunk_time + sum(r.get('transcribe_time', 0) for r in results),
...
#             f.write(f"CHUNKS: {results['segment_count']}\n\n")
...
#                 vad_marker = "[VAD]" if seg.get('vad_detected') else ""
                
#                 f.write(f"[{start_str} → {end_str}] {seg['speaker']} {vad_marker}: {text} _{label}\n")
...
#     print(f"CHUNKS: {results['segment_count']}")
#     print()
...
#         print(f"[{start_str} → {end_str}] {seg['speaker']} {vad_marker}: {text} _{label}")
...
#         f.write("="*80 + "\n")
#         f.write("AMD DETECTION WITH ENHANCED PIPELINE\n")
#         f.write("Pipeline: pyAnnote => VAD + Duration Chunks => Whisper + Text Cleaning => Classification\n")
...
#     print(f"Total chunks: {total_segments}")
#     print(f"Total HUMAN chunks: {human_count}")
#     print(f"Total MACHINE chunks: {machine_count}")
...
#         f.write(f"Total chunks: {total_segments}\n")
#         f.write(f"Total HUMAN chunks: {human_count}\n")
#         f.write(f"Total MACHINE chunks: {machine_count}\n")
...
#     print("\n" + "="*80)
#     print("AMD Detection with Enhanced Pipeline - VAD + Duration Chunking")
#     print("="*80)
#     print("Pipeline: pyAnnote => VAD + Duration Chunks => Whisper + Text Cleaning => Classification")
#     print("="*80 + "\n")
...
# MAX_CHUNK_DURATION = 5.0  # Max 5 seconds per chunk to prevent hallucinations
...
# def keyword_based_detection(text: str) -> Tuple[Optional[str], float, str]:
#     """Keyword-based AMD detection with priority matching."""
#     if not text or len(text.strip()) < 2:
#         return None, 0.0, ""
...
# print("=" * 78)
# print(" AMD Detection - VAD Only Pipeline (Optimized for CPU)")
# print(f" Device: {DEVICE}")
# print(f" Whisper: {WHISPER_MODEL_DIR}")
# print("=" * 78)
...
#     speech_start = 0
#     silence_frames = 0
#     max_silence_frames = 6  # ~180ms of silence to end segment
...
#     # Check for repeated 2-gram phrases and remove excess repetitions
#     phrase_positions = {}
#     for i in range(len(cleaned_words) - 1):
#         phrase = f"{cleaned_words[i]} {cleaned_words[i+1]}"
...
#     # Remove indices of heavily repeated phrases (keep first occurrence only)
#     skip_indices = set()
#     for phrase, positions in phrase_positions.items():
#         if len(positions) > 2:  # If phrase appears more than twice
#             # Keep first 2, skip the rest
#             for pos in positions[2:]:
#                 skip_indices.add(pos)
#                 skip_indices.add(pos + 1)
    
#     # Rebuild text without skipped words
...
# def process_audio_file(file_path: str, output_file) -> Dict:
#     """
#     Process audio file with optimized VAD-only pipeline:
#     Load => VAD Detection => Duration Chunking => Whisper + Text Cleaning => Classification
#     """
#     print(f"\n{'='*80}")
#     print(f"Processing: {os.path.basename(file_path)}")
#     print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
#             print(f"  Chunk {i}: ({chunk['start']:.2f}s-{chunk['end']:.2f}s): '{display_text}' -> {decision}")
#         else:
#             print(f"  Chunk {i}: ({chunk['start']:.2f}s-{chunk['end']:.2f}s): No speech detected")
...
#             f.write(f"{'='*80}\n")
#             f.write(f"DURATION: {results['duration']:.2f} seconds\n")
#             f.write(f"VAD SEGMENTS: {results['vad_segment_count']}\n")
#             f.write(f"CHUNKS: {results['chunk_count']}\n\n")
...
#                 text = seg['text'].strip()
#                 label = seg['decision'].lower()
#                 chunk_type = seg.get('chunk_type', 'original')
                
#                 f.write(f"[{start_str} → {end_str}] ({chunk_type}): {text} _{label}\n")
...
#         print(f"[{start_str} → {end_str}]: {text} _{label}")
...
#     with open(output_file_name, 'w', encoding='utf-8') as f:
#         f.write("="*80 + "\n")
#         f.write("AMD DETECTION - VAD OPTIMIZED PIPELINE\n")
...
# if __name__ == "__main__":
#     print("\n" + "="*80)
#     print("AMD Detection - Optimized VAD Pipeline (CPU Friendly)")
#     print("="*80 + "\n")
...
#upper VAD is working good i just need to add bconfidence . 
#!/usr/bin/env python3
#!/usr/bin/env python3
import os
import json
